...
img2mse_weighted = lambda x, y, z : torch.mean((torch.mean((x - y) ** 2,dim=1)) * z)
...
def weighted_percentile(t, w, ps):
    ...
    fn = lambda cw_i, t_i: sorted_interp(torch.tensor(ps, device=t.device) / 100, cw_i, t_i)
...
def integrate_weights(w):
  ...
  """
    cw = torch.cumsum(w, dim=-1).clamp_max(1)
    
    return cw


def filtered_loss(distance_percents,thres,factor=1):
    '''
        distance 그래프가 variance가 작게나와야한다는 가정 정도?
    '''
    residual = distance_percents[:,-1] - distance_percents[:,0]
    residual_thres = torch.clamp(residual,min = thres) + torch.clamp(residual - thres,min=0) * (factor -1)

    return (residual_thres / thres) 