...
import os, pudb
# from eval_utils import average_arrays
...
class Navigator(NavigatorBase):
    ...
    def check_if_rotation_error_good(self, return_error = False):
        ...
        if return_error:
            return rot_error
        return abs(rot_error) < acceptable_error, abs(rot_error)
...
def average_arrays(axis_list, result_dir, TYPE='position'):
    """
    Average arrays of different sizes and save the results.
    
    axis_list = [forward_passes_all, accuracy, error]
    """
    ...
    for i in range(len(axis_list[0])):
        ...
        new_axis_zs.append(np.interp(new_axis_xs[i], axis_list[0][i], axis_list[2][i]))
    ...
    midz = [np.mean([new_axis_zs[axis_idx][i] for axis_idx in range(len(axis_list[0]))]) for i in range(100)]

    # Get the current date and time
    current_time = datetime.now().strftime("%Y-%m-%d_%H-%M")

    # Create a directory with the current date and time
    result_subdir = os.path.join(result_dir, current_time)
    if not os.path.exists(result_subdir):
        os.makedirs(result_subdir)
    
    # Plotting and saving the average accuracy (midy) vs. forward passes (midx)
    plt.plot(midx, midy, label='Accuracy')
    plt.xlabel("number of forward passes (in thousands)")
    plt.ylabel("Average Accuracy")
    plt.grid()
    accuracy_plot_path = os.path.join(result_subdir, f"average_plot_{TYPE}_accuracy.png")
    plt.savefig(accuracy_plot_path)
    plt.clf()  # Clear the plot for the next graph
    
    # Plotting and saving the average error (midz) vs. forward passes (midx)
    plt.plot(midx, midz, label='Error', color='red')
    plt.xlabel("number of forward passes (in thousands)")
    plt.ylabel("Average Error")
    plt.grid()
    error_plot_path = os.path.join(result_subdir, f"average_plot_{TYPE}_error.png")
    plt.savefig(error_plot_path)
    plt.clf()  # Clear the plot for any future plots
    
    # Save the accuracy data as a text file
    accuracy_text_path = os.path.join(result_subdir, f"average_data_{TYPE}_accuracy.txt")
    with open(accuracy_text_path, mode='w') as file:
        file.write("midx\tmidy\n")  # Write the header
        for x, y in zip(midx, midy):
            file.write(f"{x}\t{y}\n")  # Write each pair of midx and midy
    
    # Save the error data as a text file
    error_text_path = os.path.join(result_subdir, f"average_data_{TYPE}_error.txt")
    with open(error_text_path, mode='w') as file:
        file.write("midx\tmidz\n")  # Write the header
        for x, z in zip(midx, midz):
            file.write(f"{x}\t{z}\n")  # Write each pair of midx and midz
...
if __name__ == "__main__":
    ...
    result_dir = rospy.get_param("result_dir")
    log_directory = rospy.get_param("log_directory")
    llff_holdout_num = rospy.get_param("llff_holdout_num")
    
    if run_inerf_compare:
        num_starts_per_dataset = 5 
        datasets = ['fern', 'horns', 'fortress', 'room'] 
        # datasets = ['fern']

        total_position_error_good = []
        total_rotation_error_good = []
        total_position_error , total_rotation_error = [],[]
        ...
        for dataset_index, dataset_name in enumerate(datasets):
            ...
            for i in range(num_starts_per_dataset):
                if not use_logged_start:
                    img_num = np.random.randint(low=0, high=10) *  llff_holdout_num # TODO can increase the range of images
                    while img_num in used_img_nums:
                        img_num = np.random.randint(low=0, high=10) *  llff_holdout_num
                    used_img_nums.add(img_num)
                
                else:
                    # pudb.set_trace()
                    start_file = start_pose_files[i]
                    img_num = int(start_file.split('_')[4])
                ...
                while num_forward_passes_per_iteration[-1] < mcl_local.forward_passes_limit:
                    ...
                    position_result = mcl_local.check_if_position_error_good()
                    rotation_result = mcl_local.check_if_rotation_error_good()
                    position_error_good.append(int(position_result[0]))
                    rotation_error_good.append(int(rotation_result[0]))
                    position_error.append(position_result[1])
                    rotation_error.append(rotation_result[1])
                    if ii != 0:
                ...
                total_position_error.append(position_error)
                total_rotation_error.append(rotation_error)

        average_arrays([total_num_forward_passes, total_position_error_good, total_position_error],result_dir,TYPE='position')
        average_arrays([total_num_forward_passes, total_rotation_error_good, total_rotation_error],result_dir,TYPE='rotation')
