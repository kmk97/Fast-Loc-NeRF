...
import time
from utils import show_img, img2mse, load_llff_data, get_pose,img2mse_weighted ,filtered_loss
...
class NeRF:
    ...
    def get_loss(self, particles, batch, photometric_loss='rgb'):
        ...
        for i in range(len(particles)):
            ...
            elif photometric_loss == 'ray_weighting':
                
                loss = img2mse(rgb, target_s) # for checking
                distance_percents_batch = extras['distance_percent'][i*len(batch): i*len(batch) + len(batch)]
                _loss = loss.clone().detach()
                loss = img2mse_weighted(rgb, target_s,filtered_loss(distance_percents_batch,thres=0.1,factor=1) )
            else:
                # TODO throw an error
